...
- 방 운영 상태: `rooms.active` — 중지하면 예약 화면(관리자/고객)에서 숨겨지고,
  예약 현황에는 '운영 중지' 뱃지로 표시된다
- 모든 도메인 테이블은 `created_at` / `updated_at`(트리거 자동 갱신) / `deleted_at`을 가지며,
  삭제는 소프트 삭제(`deleted_at` 기록)로 과거 예약이 참조하는 이름을 보존한다
